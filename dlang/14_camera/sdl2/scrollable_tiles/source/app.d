...
void main()
{
    ...
    while(gameIsRunning){
        ...
        int playerY = player.GetY();
				...
				// Scree space coordinates
				int playerXScreenSpace = player.GetX() % 640;
				int playerYScreenSpace = player.GetY() % 480;
				writeln("Screen Space: ", playerXScreenSpace, ",",playerXScreenSpace);
				writeln("World Space: ", player.GetX(), ",",player.GetY());
        ...
        if(keyboard[SDL_SCANCODE_DOWN] && moveDown){
            player.MoveDown();
				...
				// If my 'ScreenSpace is at the boundary of the world', then offset tiles
				int offsetX = player.GetX() / 640;
				int offsetY = player.GetY() / 480;
				writeln("offsets: ",offsetX, ",", offsetY);
        ...
        dt.Render(renderer,offsetX,offsetY,zoomFactor);

        // Draw our sprite
        player.Render(renderer,player.GetX() % 640,player.GetY() % 480);
