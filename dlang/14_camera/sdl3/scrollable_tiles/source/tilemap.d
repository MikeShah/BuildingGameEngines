...
struct DrawableTileMap{
  const int mMapXSize = 32;
  const int mMapYSize = 32;
    ...
    for(int y=0; y < mMapYSize; y++){
      for(int x=0; x < mMapXSize; x++){
        if(y==0){
          ...
          // Pick some random tiles otherwise to decorate the world with
          if(x % 5 == 0 && y %4 == 0){
            mTiles[x][y] = 517;
          }	
...
void Render(SDL_Renderer* renderer, int xOffset, int yOffset, int zoomFactor=1){
  for(int y=0; y < mMapYSize; y++){
    for(int x=0; x < mMapXSize; x++){
      // Avoid bounds errors
      if(x+xOffset < 0) continue;
      if(y+yOffset < 0) continue;
      if(x+xOffset > mMapXSize-1) continue;
      if(y+yOffset > mMapYSize-1) continue;


      mTileSet.RenderTile(renderer,mTiles[x+xOffset][y+yOffset],x,y,zoomFactor);
    }
  }
}
...
struct TileSet{
  ...
  void TileSetSelector(SDL_Renderer* renderer){
    ...
    int xTileSelected = cast(int)mouseX / mTileSize;
    int yTileSelected = cast(int)mouseY / mTileSize;
    ...
    // Tile to draw out on
    SDL_FRect rect = mRectTiles[tilenum];
    ...
    SDL_RenderTexture(renderer,mTexture,null,&tilemap);
    // Draw a rectangle
