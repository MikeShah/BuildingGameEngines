  ...
  void PlaySound(bool loop){
    ...
    if(!loop){
      if(SDL_GetAudioStreamQueued(mStream) < cast(int)mWaveDataLength){
        ...
        writeln(SDL_GetAudioStreamQueued(mStream));
      }
    }
    if(loop){
      // Requeue the sound so that it 'loops'
      if(SDL_GetAudioStreamQueued(mStream) < cast(int)mWaveDataLength/2){
        SDL_PutAudioStreamData(mStream,mWaveData,mWaveDataLength);	
        writeln("currently queued:",SDL_GetAudioStreamQueued(mStream), " < ", cast(int)mWaveDataLength);
      }
    }
  }
  // Loop sound forever
